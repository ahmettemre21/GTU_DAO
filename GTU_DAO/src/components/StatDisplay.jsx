...
const StatDisplay = ({ user, showDetails = false, className = '' }) => {
  ...
  const getNextTierProgress = (balance) => {
  ...
  const TooltipContent = () => (
    <div className="absolute z-10 w-64 p-3 bg-white border border-gray-200 rounded-lg shadow-lg top-full mt-2 left-0">
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">STAT Balance:</span>
          <span className="text-sm text-dao-blue">{statData.balance}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">Oy Ağırlığı:</span>
          <span className="text-sm text-green-600">{statData.balance}x</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">Tier:</span>
          <span className={`text-sm px-2 py-1 rounded ${tier.color}`}>
            {tier.name}
          </span>
        </div>
        {decayAmount > 0 && (
          <div className="text-xs text-orange-600 bg-orange-50 p-2 rounded">
            ⚠️ {decayAmount} STAT decay riski var. Aktif ol!
          </div>
        )}
      </div>
    </div>
